...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize )
	...
	-- add the stuff granted by this tech here
 	...
 	for policyBranch in GameInfo.PolicyBranchTechPrereq(string.format("TechType = '%s'", techType)) do
 		-- if this tech grants this player the ability to reveal this policy branch
 		local policyInfo = nil;
 		if policyBranch.EntryPolicy ~= nil then
 			policyInfo = GameInfo.Policies[GameInfoTypes[policyBranch.EntryPolicy]]
 		else
 			policyInfo = GameInfo.PolicyBranchTypes[GameInfoTypes[policyBranch.BranchType]]
 		end
		local buttonName = "B"..tostring(buttonNum);
		local thisButton = thisTechButtonInstance[buttonName];
		if thisButton then
			AdjustArtOnGrantedPolicyBranchButton( thisButton, policyInfo, textureSize );
			buttonNum = buttonNum + 1;
		end
 	end
...
function AdjustArtOnGrantedPolicyBranchButton( thisButton, thisPolicyInfo, textureSize )
	if thisButton then
		thisButton:SetToolTipString( Locale.ConvertTextKey("TXT_KEY_UNLOCK_SOCIAL_POLICY_BRANCH", thisPolicyInfo.Description)); 
		local textureOffset, textureSheet = IconLookup( 0, textureSize, "POLICY_ATLAS" );
		thisButton:SetTexture( textureSheet );
		thisButton:SetTextureOffset( textureOffset );
		thisButton:SetHide(false);
		
		techPediaSearchStrings[tostring(thisButton)] = Locale.ConvertTextKey( thisPolicyInfo.Description );
		thisButton:RegisterCallback( Mouse.eRClick, GetTechPedia );
	end
end
