...
function UpdateDisplay()
	...
	local policyBranchTechPrereqs  = GameInfo.PolicyBranchTechPrereq();
	local mapPolicyToTechPrereq = {}
	for row in policyBranchTechPrereqs do
		mapPolicyToTechPrereq[row.BranchType] = row.TechType
	end
--	if numUnlockedBranches > 0 then
		...
		while policyBranchInfo ~= nil and not policyBranchInfo.PurchaseByLevel do
			...
			local techPrereq = mapPolicyToTechPrereq[policyBranchInfo.Type]
			if (techPrereq ~= nil and not pTeam:IsHasTech(GameInfoTypes[techPrereq])) then
				bTechLock = true;
			end
			...
			if not player:IsPolicyBranchUnlocked( i ) then
				...
				elseif (not player:CanUnlockPolicyBranch(i)) then
					...
					elseif (bTechLock) then 
						local strTech = GameInfo.Technologies[GameInfoTypes[techPrereq]].Description
						strToolTip = strToolTip .. " " .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK_TECH", strTech);
						local strTechTitle = Locale.ConvertTextKey(strTech);
						thisButton:SetText( strTechTitle );
