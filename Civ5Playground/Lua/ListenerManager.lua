...
local bonus_last_dispensed = Utils.GetGlobalProperty("TesterBonusLastDispensed") or -1000
local bonus_dispense_freq = 20
...
ListenerManager.TestBonusListenerFactory = function(players)
	local func = function()
		...
		if (curTurn - bonus_last_dispensed) >= bonus_dispense_freq then
			...
			Utils.SetGlobalProperty("TesterBonusLastDispensed", curTurn)
			print(string.format("Next bonus: Turn #%d", curTurn + bonus_dispense_freq))
		end
	end
	return func 
end 